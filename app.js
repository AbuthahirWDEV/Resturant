const menu = [
  ...
  function cardDisplay(item){
      return `<div class="menu-item col-12 col-md-6 col-lg-4 pb-3">
      <div class="card">
    ...
    // display values in card 
  function values(category) {
    let display = document.getElementById('cardContainer')
    display.innerHTML=''
    if(category == 'all'){
      menu.map(item => display.innerHTML += cardDisplay(item))
      
    }else{
      let menuItems = menu.filter(item => item.category == category)
      menuItems.map(item =>  display.innerHTML += cardDisplay(item) )
    }
  ...
  //   console.log(input)
  //   let x = menu.filter(item => item.title.toLowerCase())
  //   console.log(x)
  //   if(x==input){
  ...
  function searchBar() {
    ...
    let filteredItems = menu.filter(item => item.title.toLowerCase().includes(input));
    console.log(filteredItems);

    if (filteredItems.length > 0) {
      
      filteredItems.forEach(item => {display.innerHTML += cardDisplay(item)})
      console.log('founded')
    } else {
      display.innerHTML='<h1>😅Oops the item not available !!!!</h1>'
    }
}

    values('all')

    // other method using eventlistener 
